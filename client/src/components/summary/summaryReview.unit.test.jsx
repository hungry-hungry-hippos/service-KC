...
describe('Rendering', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = Enzyme
      .shallow(<ZReview description={testData.description} scores={testData.scores} />);
    wrapperEmpty = Enzyme.shallow(<ZReview />);
  });

  it('uses the props correctly', () => {
    expect(wrapper.find('p').render().text()).toBe(testData.description.replace(/<.*?>/gi, ''));
  });
